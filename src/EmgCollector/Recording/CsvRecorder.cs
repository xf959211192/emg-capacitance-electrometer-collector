...
public sealed class CsvRecorder : IDisposable
{
    ...
    private bool[] _enabledChannels = Enumerable.Repeat(true, 8).ToArray();
    ...
    public void Start(string path, IReadOnlyList<bool>? enabledChannels = null)
    {
        lock (_sync)
        {
            StopCore();
            if (enabledChannels is not null && enabledChannels.Count != 8)
            {
            ...
            for (int channel = 0; channel < 8; channel++)
            {
                ...
                {
                    header.Append($",CH{channel + 1}电压_V");
                }
            }
        ...
        lock (_sync)
        {
            ...
            for (int sample = 0; sample < packet.SampleCount; sample++)
            {
                ...
                for (int channel = 0; channel < 8; channel++)
                {
                    ...
                    {
                        builder.Append(packet.GetVoltage(channel, sample).ToString("F8", CultureInfo.InvariantCulture));
                    }
                }
