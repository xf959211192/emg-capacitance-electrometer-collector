...
public sealed class ElectrostaticCsvRecorder : IDisposable
{
    private readonly object _sync = new();
    private StreamWriter? _writer;
    private DateTimeOffset? _startedAt;
    private bool[] _enabledChannels = Enumerable.Repeat(true, ElectrostaticMeasurement.ExpectedChannelCount).ToArray();
    private ElectrostaticCurrentRange _currentRange;
    ...
    public void Start(
        string path,
        IReadOnlyList<bool>? enabledChannels,
        ElectrostaticCurrentRange currentRange)
    {
        lock (_sync)
        {
            StopCore();
            if (enabledChannels is not null && enabledChannels.Count != ElectrostaticMeasurement.ExpectedChannelCount)
            {
                throw new ArgumentException("通道选择必须包含 8 个状态", nameof(enabledChannels));
            }

            _enabledChannels = enabledChannels is null
                ? Enumerable.Repeat(true, ElectrostaticMeasurement.ExpectedChannelCount).ToArray()
                : enabledChannels.ToArray();
            if (!_enabledChannels.Any(enabled => enabled))
            {
                throw new ArgumentException("至少选择一个导出通道", nameof(enabledChannels));
            }
            ...
            for (int channel = 0; channel < ElectrostaticMeasurement.ExpectedChannelCount; channel++)
            {
    ...
    public void Write(EmgPacket packet, int sampleRate)
    {
        lock (_sync)
        {
            if (_writer is null)
            ...
            var builder = new StringBuilder(packet.SampleCount * 240);
            for (int sample = 0; sample < packet.SampleCount; sample++)
            {
                ...
                for (int channel = 0; channel < ElectrostaticMeasurement.ExpectedChannelCount; channel++)
                {
                    ...
                    builder.Append(',');
                    if (channel < packet.ChannelCount)
                    {
                        builder.Append(ElectrostaticMeasurement
                            .GetValue(packet, channel, sample, _currentRange)
