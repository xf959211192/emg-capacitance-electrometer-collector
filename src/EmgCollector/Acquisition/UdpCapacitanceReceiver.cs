...
public sealed class UdpCapacitanceReceiver : IAsyncDisposable
{
    ...
    public void Start(IPAddress localAddress, int port)
    {
        if (IsRunning)
        {
            throw new InvalidOperationException("电容采集已经启动");
        }
    ...
    private async Task ReceiveLoopAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (CapacitancePacketParser.TryParse(result.Buffer, DateTimeOffset.Now, out CapacitancePacket? packet, out string error))
                {
