...
静电计模式顶部提供“通道与档位图”按钮。点击后会同时显示 IN1～IN8、OUT1～OUT8 接线位置和 CH1 拨码图；拨码从上到下为 mA、μA、nA，图片会随窗口自动缩放。
...
| 静电计 | CH1—CH8 | `WNZL_Emeter8` | `emeter1234` | 默认 1000 Hz | 8060 或 8080 | nA/μA/mA、nC、V |
...
| 通道 | 测量量 | 换算倍率 | 软件单位 |
| --- | --- | --- | --- |
| CH1 | 电流 | 三档显示值均为基础值×1000 | 随档位显示 nA、μA 或 mA |
| CH2 | 电荷 | ×10 | nC |
| CH3 | 电流 | ×1 | μA |
| CH4 | 电流 | ×0.1 | μA |
| CH5 | 电流 | ×0.01 | μA |
...
静电计 CH1 的软件档位必须与板上拨码一致。选择 nA、μA 或 mA 档后，波形纵轴和 CSV 表头会同步使用对应单位。切换档位前应先停止录制和采集。
...
- 静电计：CH1 根据档位导出 `CH1电流_nA`、`CH1电流_μA` 或 `CH1电流_mA`；其他列包括 `CH2电荷_nC`、`CH6电压_V` 等。
...
```text
12D1988F5A3A57B6185BECE1427DA12B48D313D040397664DF54499CED4C414C
```
...
- EMG、电容和静电计界面冒烟测试：通过；
- 静电计接线图弹窗打开、缩放和关闭检查：通过；
- CH1 nA、μA、mA 三档单位与换算测试：通过；
